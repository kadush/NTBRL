...
if ($arr['password'] != '80SHe5MF7gM73t4aog5Q') {
    ...
    //get array values
    $uniqueId = mysql_real_escape_string($arr['uniqueId']);
    ...
    $errorCode = mysql_real_escape_string($arr['errorCode']);
    $errorNotes = mysql_real_escape_string($arr['errorNotes']);
    ...
    if ($facilitycode == '10001') {
        $sql = "SELECT ID as recordId, Refacility as referredFrom,fullname as patientName, gender as gender, age as age,ageb as ageType, mobile as patientCell,address as patientAddress,c_no as clinicianCell,c_name as clinicianName,c_email as clinicianEmail FROM tr_sample1 where Cartridge_SN='$cartridgeSerial'  and facility='$facilitycode' LIMIT 1";
        $checkcartridge = mysql_query($sql) or die(mysql_error());
        $row_cartridge = @mysql_fetch_assoc($checkSample);

        //exit;
        if (@mysql_num_rows($checkcartridge) == 1) {
            exit;
        } else {
            ...
            $sql = "SELECT ID as recordId, Refacility as referredFrom,fullname as patientName, gender as gender, age as age,ageb as ageType, mobile as patientCell,address as patientAddress,c_no as clinicianCell,c_name as clinicianName,c_email as clinicianEmail FROM tr_sample1 where lab_no='$sampleid' and facility='$facilitycode' LIMIT 1";
           ...
           */
                $insert = "INSERT INTO tr_sample1 (`unq_id`,`System_Name`, `Exported_Date`, `Reagent_Lot_Number`, `Sample_ID`, `Patient_ID`, `Assay`, `Assay_Version`, `User`, `Status`, `Start_Time`, `End_Time`, `Error_Status`, `Error_Code`, `Error_Notes`, `Expiration_Date`, `Cartridge_SN`, `Module_Name`, `Module_SN`, `Instrument_SN`, `SW_Version`,`GXSN`, `Test_Result`, `mtbRif`,`facility`,`Refacility`,`cond`) 
             VALUES('" . $uniqueId . "','" . $systemName . "','" . $exportedDate . "','" . $reagentLotId . "','" . $sampleid . "','" . $patientId . "','" . $assay . "','" . $assayVersion . "','" . $user . "','" . $status . "','" . $startTime . "','" . $endTime . "','" . $errorStatus . "','" . $errorCode . "','" . $errorNotes . "','" . $cartridgeExpirationDate . "','" . $cartridgeSerial . "','" . $moduleName . "','" . $moduleSerial . "','" . $instrumentSerial . "','" . $softwareVersion . "','" . $deviceSerial . "','" . $resultIdMtb . "','" . $resultIdRif . "','" . $facilitycode . "','" . $facilitycode . "','1')";
               ...
               $where="cond='1' WHERE lab_no='".$arr['sampleId']."'";*/
                ...
                if (!$retval) {
                    ...
                    $query_rssamp = "SELECT * FROM tr_sample1 WHERE lab_no='$SampleID'";
                    $rssamp = mysql_query($query_rssamp) or die(mysql_error());
                    ...
                    if ($days > 48) { } else {
                        ...
                        if ($row_rssamp['sms_status'] == 1) { } else {

                            if ($row_rssamp['c_no'] == '2547________' or $row_rssamp['c_no'] == '' or $row_rssamp['c_no'] == '2547') { } else {

                                if ($mtb == 'Positive') {

                                    echo $x = file_get_contents("http://api.prsp.tangazoletu.com/?User_ID=13863&passkey=53P3YMG3&service=1&sender=21645&dest=$cphone&msg=Genexpert+Test+for+$pname+done+at+$fname.+%0aThe+Summaries+are;Op/Ip+No:$opno,MobileNo:$mobile,Age:$age,MTB:$mtb,RIF:$mtbrif");
                                } elseif ($mtb == 'Negative') {

                                    echo $x = file_get_contents("http://api.prsp.tangazoletu.com/?User_ID=13863&passkey=53P3YMG3&service=1&sender=21645&dest=$cphone&msg=Genexpert+Test+for+$pname+done+at+$fname.+%0aThe+Summaries+are;Op/Ip+No:$opno,MobileNo:$mobile,Age:$age,MTB:$mtb");
                                } else { }
                            }
                            if ($row_rssamp['s_no'] == '2547________' or $row_rssamp['s_no'] == '' or $row_rssamp['s_no'] == '2547') { } else {


                                if ($mtb == 'Positive') {

                                    echo $x = file_get_contents("http://api.prsp.tangazoletu.com/?User_ID=13863&passkey=53P3YMG3&service=1&sender=21645&dest=$sphone&msg=Genexpert+Test+for+$pname+done+at+$fname.+%0aThe+Summaries+are;Op/Ip+No:$opno,MobileNo:$mobile,Age:$age,MTB:$mtb,RIF:$mtbrif");
                                } elseif ($mtb == 'Negative') {

                                    echo $x = file_get_contents("http://api.prsp.tangazoletu.com/?User_ID=13863&passkey=53P3YMG3&service=1&sender=21645&dest=$sphone&msg=Genexpert+Test+for+$pname+done+at+$fname.+%0aThe+Summaries+are;Op/Ip+No:$opno,MobileNo:$mobile,Age:$age,MTB:$mtb");
                                } else { }
                            }

                            if ($x) {

                                $query_rssamp = "UPDATE tr_sample1 SET sms_status=1 WHERE lab_no='$SampleID'";
                                $rssamp = mysql_query($query_rssamp) or die(mysql_error());
                            } else { }
                        }
                        //end
                        ...
                        if ($row_rssamp['email_status'] == 1) { } else {
                           ...
                           GeneXpert test for " . $row_rssamp['fullname'] . " was successfully done. Please notify the patient to visit the facility where the test was done from to collect the results. Below are the test summaries.<br><br/>
                           
                                   Lab No : " . $row_rssamp['lab_no'] . " <br><br/>
                                   Date Tested : " . $row_rssamp['coldate'] . " <br><br/>
                                   Patient Name : " . $row_rssamp['fullname'] . " <br><br/>
                                   Gender : " . $row_rssamp['gender'] . " <br><br/>
                                   Age : " . $row_rssamp['age'] . " <br><br/>
                                   Patient Type : " . $row_rssamp['pat_type'] . " <br><br/>
                                   Mobile No : " . $row_rssamp['mobile'] . " <br><br/>
                                   MTB Result: " . $row_rssamp['Test_Result'] . " <br><br/>
                                   MTB Rif: " . $row_rssamp['mtbRif'] . " <br><br/>
                                   Testing Facility: " . $tname1 . " <br><br/>
                                   Referring Facility: " . $fname1 . " <br><br/>
                                   Technician : " . $row_rssamp['User'] . " .<br><br/>
                               
                           Many Thanks.<br><br/>
                           -- <br><br/>
                           TB Support Team <br><br/>
                           This email was automatically generated. Please do not respond to this email address since it will be ignored.";

                                if (!$mail->Send()) { } else {

                                    $query_rssamp = "UPDATE tr_sample1 SET email_status=1 WHERE lab_no='$SampleID'";
                                    $rssamp = mysql_query($query_rssamp) or die(mysql_error());
                                }
                            }
                        }
			...
			*/
			  ...
			  VALUES('" . $uniqueId . "','" . $systemName . "','" . $exportedDate . "','" . $reagentLotId . "','" . $sampleid . "','" . $patientId . "','" . $assay . "','" . $assayVersion . "','" . $user . "','" . $status . "','" . $startTime . "','" . $endTime . "','" . $errorStatus . "','" . $errorCode . "','" . $errorNotes . "','" . $cartridgeExpirationDate . "','" . $cartridgeSerial . "','" . $moduleName . "','" . $moduleSerial . "','" . $instrumentSerial . "','" . $softwareVersion . "','" . $deviceSerial . "','" . $resultIdMtb . "','" . $resultIdRif . "','" . $facilitycode . "','" . $facilitycode . "','1')";
                ...
                if (!$retval) {
                    ...
                    $query_rssamp = "SELECT * FROM sample1 WHERE lab_no='$SampleID'";
                    ...
                    if ($days > 48) { } else {
                        ...
                        if ($row_rssamp['email_status'] == 1) { } else {
							...
							Below are the test summaries for GeneXpert test having Positive outcomes on both MTB and RIF.<br><br/>
							        
							        Full Name: " . $row_rssamp['fullname'] . "<br><br/>
								    Lab No : " . $row_rssamp['lab_no'] . " <br><br/>
									Date Tested : " . $row_rssamp['coldate'] . " <br><br/>
									Patient Name : " . $row_rssamp['fullname'] . " <br><br/>
									Gender : " . $row_rssamp['gender'] . " <br><br/>
									...
									MTB Result: " . $row_rssamp['Test_Result'] . " <br><br/>
									MTB Rif: " . $row_rssamp['mtbRif'] . " <br><br/>
									Testing Facility: " . $tname . " <br><br/>
									Referring Facility: " . $fname . " <br><br/>
									Technician : " . $row_rssamp['User'] . " .<br><br/>
            ...
            echo json_encode($row_rsFinC);
            //echo $x= file_get_contents("http://api.prsp.tangazoletu.com/?User_ID=13863&passkey=53P3YMG3&service=1&sender=21645&dest=254724690198&msg=success+API.");
