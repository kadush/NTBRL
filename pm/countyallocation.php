...
ORDER BY `c`.`facility`
";
$rsallctn = mysqli_query($dbConn,$sql) or die(mysqli_error($dbConn));
$row_rsallctn = mysqli_fetch_assoc($rsallctn);

$query = "SELECT s.id, s.facility, s.to_fac, s.date, s.f_quantity, s.c_quantity, s.adjustment
FROM st_view s
WHERE s.county = '$countyID'";
$rssam = mysqli_query($dbConn,$query) or die(mysqli_error($dbConn));
$row = mysqli_fetch_assoc($rssam);
$total1 = mysqli_num_rows($rssam);
$TT=TOTALFacilityReportedpercounty($countyID); 
		...
		<div class="panel panel-gradient">
			<div class="panel-heading">
				<div class="panel-title">
					<h4>
						Genexpert County Allocation For <?php echo $countyID; ?> County <!-- <span class="badge badge-info badge-roundless" style="float: right" ><?php echo $TT.' / '.$TT1; ?></span> -->
					</h4>
				</div>
			...
			<div class="panel-body no-padding">
				<table class="table table-bordered" id="table-1">
					    ...
					    <?php if( @mysqli_num_rows($rsallctn)==0)
                                 {
				        ...
				        <?php } while ($row_rsallctn = mysqli_fetch_assoc($rsallctn)); } ?> 
			 ...
			 <table class="table table-bordered datatable" id="table-2">
					...
					if ($total1==0) {
					   ...
					   do
						...
						<?php }  while ($row = mysqli_fetch_assoc($rssam));
					} ?> 				
