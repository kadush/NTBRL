...
if ($countyID==0) {//all county
	if ($qr==0) {
		...
		$sql="SELECT * from eqa where quarter='$qr' and year='$yr'";
		}
	} else { //all districts in county
		
		if ($sc==0) {
			if ($qr==0) {
			$sql="SELECT year, facility, fname, district, county, SUM(ml_pos) AS ml_pos, SUM(ml_scanty) AS ml_scanty, SUM(ml_neg) AS ml_neg, SUM(ml_hfp) AS ml_hfp, SUM(ml_lfp) AS ml_lfp, SUM(ml_hfn) AS ml_hfn, SUM(ml_lfn) AS ml_lfn, SUM(ml_qe) AS ml_qe, SUM(fc_pos) AS fc_pos, SUM(fc_scanty) AS fc_scanty, SUM(fc_neg) AS fc_neg, SUM(fc_hfp) AS fc_hfp, SUM(fc_lfp) AS fc_lfp, SUM(fc_hfn) AS fc_hfn, SUM(fc_lfn) AS fc_lfn, SUM(fc_qe) AS fc_qe,rec_lab,un_lab from eqa where year='$yr' and countyid='$countyID' group by fname";
			
			} else {
			$sql="SELECT * from eqa where quarter='$qr' and year='$yr' and countyid='$countyID'";
			}
					
		} else {
			if ($qr==0) {
				$sql="SELECT year, facility, fname, district, county, SUM(ml_pos) AS ml_pos, SUM(ml_scanty) AS ml_scanty, SUM(ml_neg) AS ml_neg, SUM(ml_hfp) AS ml_hfp, SUM(ml_lfp) AS ml_lfp, SUM(ml_hfn) AS ml_hfn, SUM(ml_lfn) AS ml_lfn, SUM(ml_qe) AS ml_qe, SUM(fc_pos) AS fc_pos, SUM(fc_scanty) AS fc_scanty, SUM(fc_neg) AS fc_neg, SUM(fc_hfp) AS fc_hfp, SUM(fc_lfp) AS fc_lfp, SUM(fc_hfn) AS fc_hfn, SUM(fc_lfn) AS fc_lfn, SUM(fc_qe) AS fc_qe,rec_lab,un_lab from eqa where year='$yr' and districtid='$DistrictID' group by fname";
				
			} else {
			$sql="SELECT * from eqa where quarter='$qr' and year='$yr' and districtid='$DistrictID'";
			}
		}
